...
image:https://img.shields.io/maven-central/v/io.github.wimdeblauwe/testcontainers-cypress[Maven Central Version]

📕 Want to write production-ready Spring Boot starters? Read https://leanpub.com/crafting-spring-boot-starters[Crafting Spring Boot Starters].
...
. Run `npm install cypress@15.10.0 --save-dev` to install Cypress as a development dependency. You can also use a different version of Cypress, just make sure that you provide the same version in the `CypressContainer` constructor to avoid compatibility issues.
...
|withNpmRunArguments(String args)
|Adds additional arguments to the `npm install` command. For example, you can add `--loglevel silent`.
|
|===
...
|https://github.com/wimdeblauwe/testcontainers-cypress/releases/tag/testcontainers-cypress-1.10.0[1.10.0]
|https://github.com/testcontainers/testcontainers-java/releases/tag/1.21.4[1.21.4]
|https://docs.cypress.io/guides/references/changelog.html#15-10-0[15.10.0]
